...
export default function Navbar() {
    ...
    const fetchUserImage = async () =>{
        ...
        if (user != null) {
            await axios
                .get(process.env.NEXT_PUBLIC_API_ENDPOINT + API_ENDPOINTS.AgentViewProfileImage, {
    ...
    const signOut = async () => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.get(
                process.env.NEXT_PUBLIC_API_ENDPOINT + API_ENDPOINTS.AgentAuthLogout,
                {
                    withCredentials: true,
                }
            );
                                        ...
                                        <div className="w-full bg-white shadow-2xl transform transition-transform ease-in-out duration-500 rounded-lg">
                                            ...
                                            <div className="py-2" aria-labelledby="user-menu-button">
                                                <a
                                                    ...
                                                    onClick={(e) => { e.preventDefault(); navigate(routes.Agent_dashboard); }}
                                                >
                                                    Dashboard
                                                </a>
                                                <a
                                                    href="#"
                                                    className="block px-4 py-3 text-sm text-gray-800 hover:bg-gray-100 hover:text-gray-900 transition duration-300 rounded-md"
                                                    onClick={(e) => { e.preventDefault(); navigate(routes.Agent_profile); }}
                                                >
                                                    My Profile
                                                </a>
                                                <a
