...
export class AgentService {
  ...
  get_service(): string {
    return 'AgentService is working!';
  }
  ...
  async Create_Agent(Agent_info: AgentDto): Promise<any> {
    try {
      const AgentEntity = this.mapperService.dtoToEntity(
        Agent_info,
        agentEntity,
      );

      const user = await this.userRepository.findOneBy({
        id: Agent_info.user_id,
      });

      AgentEntity.user = user;
      AgentEntity.image = 'temp.svg';
      AgentEntity.credit_amount = 0;
      const number_existancy = await this.AgentRepository.findOneBy({
        phone: AgentEntity.phone,
      });
      if (number_existancy == null) {
        const saved_Agent =
          await this.AgentRepository.save(AgentEntity);
        return saved_Agent ? saved_Agent.id : -1;
      } else {
        throw new InternalServerErrorException('Phone Already exists');
      }
    } catch (e) {
      throw new InternalServerErrorException(
        'Agent Service, Create Agent Error = ' + e.message,
      );
    }
  }

  async Find_Agent_By_Email(email: string): Promise<any> {
    const user = await this.userRepository.findOneBy({ email: email });
    const Agent = await this.AgentRepository.findOneBy({ user: user });
    //   Convert to Agent Profile
    const Agent_Profile_DTO = this.mapperService.entityToDto(
      Agent,
      Agent_ProfileDTO,
    );

    Agent_Profile_DTO.id = user.id;
    Agent_Profile_DTO.email = user.email;
    return Agent_Profile_DTO;
  }

  async Update_Own_Profile_Details(
    email: string,
    updated_data: Agent_ProfileDTO,
  ): Promise<any> {
    try {
      const previous_data = await this.Find_Agent_By_Email(email);
      const previous_user = await this.userRepository.findOneBy({
        email: email,
      });
      const previous_Agent = await this.AgentRepository.findOneBy({
        user: previous_user,
      });
      ...
      if (
        ...
        await this.AgentRepository.update(previous_Agent.id, {
  ...
  async Add_Credits_Amount(
    ...
    try {
      ...
      const user = await this.userRepository.findOneBy({ email: email });
      const Agent = await this.AgentRepository.findOneBy({ user: user });
      const final_amount = Agent.credit_amount + amount;
  ...
  async Subtract_Credits_Amount(
    ...
    try {
      ...
      const user = await this.userRepository.findOneBy({ email: email });
      const Agent = await this.AgentRepository.findOneBy({ user: user });
      console.log('Current amount = ' + Agent.credit_amount);
      if (Agent.credit_amount > 0) {
        const final_amount =
          Agent.credit_amount -
  ...
  async Update_Profile_Picture(
    email: string,
    image: string,
  ): Promise<Agent_ProfileDTO> {
    const current_user = await this.userRepository.findOneBy({ email: email });
    const current_Agent = await this.AgentRepository.findOneBy({
      user: current_user,
    });

    if (current_Agent) {
      (await current_Agent).image = image;
      await this.AgentRepository.update((await current_Agent).id, {
        image: image,
      });
    }

    return await this.Find_Agent_By_Email(email);
  }
  ...
  async Get_Profile_Picture(email: string, res: any): Promise<any> {
    ...
    if (current_Agent) {
      res.sendFile((await current_Agent).image, {
  ...
  async Create_Payment(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Agent Service, Create Payment Error = ' + e.message,
      );
    }
  }
  ...
  async Update_Password(req: Request, password: string): Promise<any> {
    try {
      ...
      throw new InternalServerErrorException(
        'Update Password Agent Service error = ' + e.message,
      );
    }
  }
  ...
  async navbar(email: string): Promise<any> {
    ...
    const navbarInfo: NavbarDTO = {
      name: Agent.name,
      credit_amount: Agent.credit_amount,
    };
  ...
  private extractTokenFromHeader(request: Request): string | undefined {
    try {
      ...
      throw new InternalServerErrorException(
        'extract Token From Header Agent service error = ' + e.message,
      );
    }
  }
  ...
  async get_user_from_Request(req: Request): Promise<UserEntity> {
    try {
      ...
      throw new InternalServerErrorException(
        'Get user from request Agent service error = ' + e.message,
      );
    }
  }
  ...
  async updateAgent_SingleInfo(id: number, column: string, data: any) {
    const updateData = {};
  ...
  async user_validity(email: string, password: string): Promise<boolean> {
    try {
      ...
      throw new InternalServerErrorException(
        'Agent Service, user validity Error = ' + e.message,
      );
    }
  }
