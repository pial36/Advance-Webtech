...
import { LoginDTO } from '../Agent.dto';
...
export class AuthService {
  ...
  async signUp(myobj: LoginDTO): Promise<any> {
    return await this.AgentService.Create_Signup(myobj);
  }
  async signIn(logindata: LoginDTO): Promise<{ access_token: string }> {
    const user = await this.AgentService.Login(logindata);
    if (!user) {
  ...
  async UpdatePassword(req: Request, password: string): Promise<any> {
    try {
      const decision = await this.AgentService.Update_Password(
        req,
        password,
      );
      if (decision !== null) {
  ...
  async destroy_temporary_JWT(req: Request): Promise<any> {
    try {
      ...
      const user = await this.AgentService.get_user_from_Request(req);
