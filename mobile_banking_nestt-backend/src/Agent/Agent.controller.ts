...
export class AgentController {
  ...
  @HttpCode(HttpStatus.OK) // Set the status code to 200 (OK)
  getService(): any {
    return this.AgentService.get_service();
  }
   ...
   * @param {AgentDto} x - The first number.
   * @returns {any} The sum of x and y.
   */

  //region Authentication

  @Post('/signup/Agent_details')
  @UsePipes(new ValidationPipe())
  @HttpCode(HttpStatus.OK) // Set the status code to 200 (OK)
  async _Details_Create(
    @Body() Agent_info: AgentDto,
  ): Promise<any> {
    try {
      const saved_Agent =
        await this.AgentService.Create_Agent(Agent_info);
      if (saved_Agent > 0) {
        return saved_Agent;
      } else {
        throw new InternalServerErrorException(
          'Agent data could not be saved',
        );
      }
    } catch (e) {
      throw new InternalServerErrorException({
  ...
  async View_own_Profile(@Request() req): Promise<any> {
    try {
      return await this.AgentService.Find_Agent_By_Email(req.user.email);
    } catch (e) {
  ...
  async Update_own_Profile(
    @Request() req,
    @Body() updated_data: Agent_ProfileDTO,
  ): Promise<any> {
    try {
      return await this.AgentService.Update_Own_Profile_Details(
        req.user.email,
        updated_data,
      );
    } catch (e) {
  ...
  async UploadProfileImage(
    ...
    const seller = await this.AgentService.Update_Profile_Picture(
      req.user.email,
      myfileobj.filename,
    );
    if (seller != null) {
  ...
  async getSellerImages(@Request() req, @Res() res): Promise<any> {
    try {
      return this.AgentService.Get_Profile_Picture(req.user.email, res);
    } catch (e) {
      throw new InternalServerErrorException(e.message);
    }
  }
  ...
  async Create_Billing(
    ...
    try {
      ...
      const user_validity_decision = await this.AgentService.user_validity(
        req.user.email,
        bill.password,
      );
      ...
      if (user_validity_decision) {
        const receiver = await this.AgentService.getUserByEmail(
          bill.receiver_info,
        );
        if (receiver != null) {
          bill.payment_type = 'Send Money';
          const decision = await this.AgentService.Subtract_Credits_Amount(
            req.user.email,
            bill,
          );
          ...
          const decision_add = await this.AgentService.Add_Credits_Amount(
      ...
      throw new InternalServerErrorException(
        'Agent Controller Create Billing Error = ' + e.message,
      );
    }
  }
  ...
  async Cash_In(
    ...
    try {
      ...
      if (user_validity_decision) {
        ...
        const decision = await this.AgentService.Add_Credits_Amount(
          req.user.email,
          bill,
        );
  ...
  async Get_All_Billing(@Request() req): Promise<any> {
    try {
      const payment_list = this.AgentService.Get_All_Billing_Payment(
        req.user.email,
      );
  ...
  async Get_All_Cash_in(@Request() req): Promise<any> {
    try {
      const payment_list =
        await this.AgentService.Get_All_Billing_Payments_By_Payment_Type(
          req.user.email,
          'Cash in',
        );
  ...
  async Forget_Password(
    ...
    try {
      const decision = await this.AgentService.ForgetPassword(
  ...
  async OTP_Verification(
    ...
    try {
      ...
      const decision = await this.AgentService.otp_verification(
        req,
        OTP_Object.otp,
      );
      if (decision) {
  ...
  async Navbar(@Request() req): Promise<any> {
    ...
    try {
      const navbar_data = await this.AgentService.navbar(req.user.email);
      // console.log('Request Headers:', req.headers);
      console.log('Agent_name = ' + navbar_data.name);
      console.log('Agent money = ' + navbar_data.credit_amount);
