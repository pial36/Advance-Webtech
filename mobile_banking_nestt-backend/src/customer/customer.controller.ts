...
export class CustomerController {
   ...
   * @param {CustomerDto} x - The first number.
   * @returns {any} The sum of x and y.
   */

  //region Authentication

  @Post('/signup/customer_details')
  @UsePipes(new ValidationPipe())
  @HttpCode(HttpStatus.OK) // Set the status code to 200 (OK)
  async customer_Details_Create(
    @Body() customer_info: CustomerDto,
  ): Promise<any> {
    try {
      const saved_customer =
        await this.customerService.Create_Customer(customer_info);
      if (saved_customer > 0) {
  ...
  async Update_own_Profile(
    @Request() req,
    @Body() updated_data: Customer_ProfileDTO,
  ): Promise<any> {
    try {
      return await this.customerService.Update_Own_Profile_Details(
        req.user.email,
        updated_data,
      );
    } catch (e) {
  ...
  async UploadProfileImage(
    ...
    const seller = await this.customerService.Update_Profile_Picture(
      req.user.email,
      myfileobj.filename,
    );
    if (seller != null) {
  ...
  async Create_Billing(
    ...
    try {
      ...
      const user_validity_decision = await this.customerService.user_validity(
        req.user.email,
        bill.password,
      );
      ...
      if (user_validity_decision) {
        const receiver = await this.customerService.getUserByEmail(
          bill.receiver_info,
        );
        if (receiver != null) {
          bill.payment_type = 'Send Money';
          const decision = await this.customerService.Subtract_Credits_Amount(
            req.user.email,
            bill,
          );
          ...
          const decision_add = await this.customerService.Add_Credits_Amount(
      ...
      throw new InternalServerErrorException(
        'Customer Controller Create Billing Error = ' + e.message,
      );
    }
  }
  ...
  async Cash_In(
    ...
    try {
      ...
      if (user_validity_decision) {
        ...
        const decision = await this.customerService.Add_Credits_Amount(
          req.user.email,
          bill,
        );
  ...
  async Get_All_Billing(@Request() req): Promise<any> {
    try {
      const payment_list = this.customerService.Get_All_Billing_Payment(
        req.user.email,
      );
  ...
  async Get_All_Cash_in(@Request() req): Promise<any> {
    try {
      const payment_list =
        await this.customerService.Get_All_Billing_Payments_By_Payment_Type(
          req.user.email,
          'Cash in',
        );
  ...
  async Forget_Password(
    ...
    try {
      const decision = await this.customerService.ForgetPassword(
  ...
  async OTP_Verification(
    ...
    try {
      ...
      const decision = await this.customerService.otp_verification(
        req,
        OTP_Object.otp,
      );
      if (decision) {
