...
export class MerchantService {
  ...
  async Create_Merchant(merchant_info: MerchantDto): Promise<any> {
    try {
      ...
      merchantEntity.image = 'temp.svg';
      merchantEntity.credit_amount = 0;
  ...
  async Add_Credits_Amount(
    ...
    try {
      ...
      const user = await this.userRepository.findOneBy({ email: email });
      const merchant = await this.merchantRepository.findOneBy({ user: user });
      const final_amount = merchant.credit_amount + amount;
  ...
  async Subtract_Credits_Amount(
    ...
    try {
      ...
      const user = await this.userRepository.findOneBy({ email: email });
      const merchant = await this.merchantRepository.findOneBy({ user: user });
      console.log('Current amount = ' + merchant.credit_amount);
      if (merchant.credit_amount > 0) {
        const final_amount =
          merchant.credit_amount -
  ...
  async Get_Profile_Picture(email: string, res: any): Promise<any> {
    ...
    if (current_merchant) {
      res.sendFile((await current_merchant).image, {
        root: './uploaded_pic/profile_images',
      });
    } else {
  ...
  async navbar(email: string): Promise<any> {
    ...
    const navbarInfo: NavbarDTO = {
      name: merchant.name,
      credit_amount: merchant.credit_amount,
    };
